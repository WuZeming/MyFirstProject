...
const BinaryTreeNode* KthNodeCore(const BinaryTreeNode* pRoot, unsigned int& k)
{
	const BinaryTreeNode* result = nullptr;
	if (pRoot->m_pLeft != nullptr)
	{
		result = KthNodeCore(pRoot->m_pLeft, k);
	}
	if (result == nullptr)
	{
		if (k == 1)
	...
	if (result == nullptr && pRoot->m_pRight!= nullptr)
	{
		result = KthNodeCore(pRoot->m_pRight, k);
	}
...
const BinaryTreeNode* KthNode2(const BinaryTreeNode* pRoot, unsigned int k)
{
	...
	const BinaryTreeNode* currNode = pRoot;
