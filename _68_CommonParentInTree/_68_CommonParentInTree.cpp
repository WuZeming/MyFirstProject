...
bool GetNodePath(const TreeNode* pRoot, const TreeNode* pNode, list<const TreeNode*>& path)
{
	...
	for (auto i :(pRoot->m_vChildren))
	{
		if (GetNodePath(i, pNode, path))
		{
			find = true;
			break;
		}
...
{
	list<const TreeNode*>::const_iterator begin1 = path1.begin();
	list<const TreeNode*>::const_iterator begin2 = path2.begin();
	const TreeNode* last = nullptr;
	while (begin1!=path1.end()&&begin2 != path2.cend())
	{
		if (*begin1 == *begin2)
		{
			last = *begin1;
